...
OUTCOME_CARD = '''<div class="outcome-activity-row"><div class="card" id="outcomeCard"><div class="title">Результаты реализации</div><div class="note" style="margin-bottom:10px">Успешно реализовано и причины «Закрыто и не реализовано» за выбранный период.</div><div class="hbars" id="outcomes"></div></div>'''
...
#pipelineActivityCard .pipeline-activity-head .title{margin:0;white-space:nowrap}
.pipeline-activity-note{margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.pipeline-activity-grid{display:grid;grid-template-columns:minmax(88px,1.45fr) repeat(7,minmax(20px,.55fr)) minmax(26px,.6fr);gap:2px;width:100%;min-width:0;align-items:stretch}
...
@media(max-width:760px){.outcome-activity-row{grid-template-columns:1fr;gap:8px}#pipelineActivityCard{padding:8px}.pipeline-activity-head{gap:4px}.pipeline-week-control{width:auto;justify-content:flex-end}.pipeline-week-control select{flex:0 1 165px;max-width:165px}.pipeline-activity-grid{grid-template-columns:minmax(105px,1.45fr) repeat(7,minmax(24px,.55fr)) minmax(32px,.6fr);gap:2px;width:100%}.pa-head,.pa-label,.pa-cell,.pa-total{height:11px;min-height:11px;border-radius:2px}.pa-head,.pa-label,.pa-cell,.pa-total{font-size:7px}.pa-label{padding:0 2px}.pipeline-activity-note{white-space:normal}}
'''
