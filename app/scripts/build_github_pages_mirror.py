...
async function waitForPublicRefresh(created,previous,serial,runId){var deadline=Date.now()+20*60*1000,lastError=null,run=runId?{id:runId}:null,workflowDone=false,payload=null,snap='';while(Date.now()<deadline){if(serial!==refreshSerial)throw new Error('refresh_cancelled');try{run=await findPublicRun(created,run&&run.id);if(run){if(run.status==='queued'||run.status==='pending'||run.status==='waiting'||run.status==='requested')progress(1,'Проверка системы…');else if(run.status==='in_progress')progress(2,'Сбор Telegram/MAX + amoCRM…');else if(run.status==='completed'){if(run.conclusion!=='success')throw new Error('workflow_failed');workflowDone=true;progress(3,'Сборка дашборда…')}}}catch(e){lastError=e;if(String(e.message||e)==='workflow_failed')throw e}try{payload=await loadDashboardPayload();snap=snapshotValue(payload);if(workflowDone&&snapshotIsNewer(snap,previous)){progress(4,'Сохранение данных…');return payload}}catch(e){lastError=e}await sleep(5000)}throw lastError||new Error('refresh_timeout')}
async function refreshDashboard(){if(refreshInProgress)return;refreshInProgress=true;var btn=id('refreshBtn'),previous=currentSnapshot,serial=++refreshSerial;btn.disabled=true;btn.textContent='Обновление…';progressStart();try{try{var baseline=snapshotValue(await loadDashboardPayload());if(snapshotIsNewer(baseline,previous))previous=baseline}catch(e){}progress(1,'Проверка системы…');var start=await ensurePublicWorkflow();if(start.reused)progress(1,'Обновление уже запущено…');var payload=await waitForPublicRefresh(start.created,previous,serial,start.runId);if(serial!==refreshSerial)throw new Error('refresh_cancelled');if(!applyDashboard(payload))throw new Error('render_failed');setRefreshState('Обновлено '+new Date().toLocaleTimeString('ru-RU',{timeZone:'Europe/Moscow',hour:'2-digit',minute:'2-digit'}),false);progressStop(true)}catch(e){setRefreshState('Ошибка обновления',true);progressStop(false)}finally{refreshSerial++;btn.disabled=false;btn.textContent='Обновить данные';refreshInProgress=false}}
"""
