...
async function dispatchPublicWorkflow(){var created=new Date().toISOString();await gh('/actions/workflows/'+encodeURIComponent(GH_WORKFLOW)+'/dispatches',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ref:GH_REF})});return created}
async function findPublicRun(created,runId){var data=await gh('/actions/workflows/'+encodeURIComponent(GH_WORKFLOW)+'/runs?branch='+encodeURIComponent(GH_REF)+'&event=workflow_dispatch&per_page=20'),runs=data.workflow_runs||[],after=Date.parse(created)-120000,candidate=null,c,i;for(i=0;i<runs.length;i++){if(runId&&String(runs[i].id)===String(runId))return runs[i];c=Date.parse(runs[i].created_at||'');if(runs[i].event==='workflow_dispatch'&&!isNaN(c)&&c>=after&&!candidate)candidate=runs[i]}return candidate}
async function waitForPublicRefresh(created,previous,serial){var deadline=Date.now()+20*60*1000,lastError=null,run=null,workflowDone=false,payload=null,snap='';while(Date.now()<deadline){if(serial!==refreshSerial)throw new Error('refresh_cancelled');try{run=await findPublicRun(created,run&&run.id);if(run){if(run.status==='queued')progress(1,'Проверка системы…');else if(run.status==='in_progress')progress(2,'Сбор Telegram/MAX + amoCRM…');else if(run.status==='completed'){if(run.conclusion!=='success')throw new Error('workflow_failed');workflowDone=true;progress(3,'Сборка дашборда…')}}}catch(e){lastError=e;if(String(e.message||e)==='workflow_failed')throw e}try{payload=await loadDashboardPayload();snap=snapshotValue(payload);if(snapshotIsNewer(snap,previous)){progress(4,'Сохранение данных…');return payload}if(workflowDone&&payload){progress(4,'Сохранение данных…');return payload}}catch(e){lastError=e}await sleep(5000)}throw lastError||new Error('refresh_timeout')}
async function refreshDashboard(){if(refreshInProgress)return;refreshInProgress=true;var btn=id('refreshBtn'),previous=currentSnapshot,serial=++refreshSerial;btn.disabled=true;btn.textContent='Обновление…';progressStart();try{if(!previous){try{previous=snapshotValue(await loadDashboardPayload())||previous}catch(e){}}progress(1,'Ожидание обновления системы…');var created=await dispatchPublicWorkflow();var payload=await waitForPublicRefresh(created,previous,serial);if(serial!==refreshSerial)throw new Error('refresh_cancelled');if(!applyDashboard(payload))throw new Error('render_failed');setRefreshState('Обновлено '+new Date().toLocaleTimeString('ru-RU',{timeZone:'Europe/Moscow',hour:'2-digit',minute:'2-digit'}),false);progressStop(true)}catch(e){setRefreshState('Ошибка обновления',true);progressStop(false)}finally{refreshSerial++;btn.disabled=false;btn.textContent='Обновить данные';refreshInProgress=false}}"""
